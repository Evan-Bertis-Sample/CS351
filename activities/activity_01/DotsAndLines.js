...
// first, create a JavaScript typed array with all our vertex attribute values:
  ...
  // we are drawing lines, so each face should start with the ending vertex of the previous face
  let vertsRaw = [
    // front face, ordered clockwise
    -0.5, 0.5, 0.5, 1.0, // front upper left
    0.5, 0.5, 0.5, 1.0, // front upper right
    0.5, -0.5, 0.5, 1.0, // front lower right
    -0.5, -0.5, 0.5, 1.0, // front lower left
    // left face, starting from front upper left
    -0.5, 0.5, 0.5, 1.0, // front upper left
    -0.5, 0.5, -0.5, 1.0, // back upper left
    -0.5, -0.5, -0.5, 1.0, // back lower left
    -0.5, -0.5, 0.5, 1.0, // front lower left
    // bottom face, starting from front lower left
    -0.5, -0.5, -0.5, 1.0, // back lower left
    0.5, -0.5, -0.5, 1.0, // back lower right
    0.5, -0.5, 0.5, 1.0, // front lower right
    // back face, starting from front lower right
    0.5, 0.5, 0.5, 1.0, // front upper right
    0.5, 0.5, -0.5, 1.0, // back upper right
    0.5, -0.5, -0.5, 1.0, // back lower right
    // now draw the last edge
    0.5, 0.5, -0.5, 1.0, // back upper right 
    -0.5, 0.5, -0.5, 1.0 // back upper left
  ]

  // I am aware that projection isn't done like this
  // this is a bit of a hack, but scale the vertices x and y values by their z distance from the origin
  // this will make the cube look like it's in perspective
  let perspectiveScaleFactor = 0.75;
  for (let i = 0; i < vertsRaw.length; i += 4) {
    scaleFactor = 1.0 + vertsRaw[i + 2] * perspectiveScaleFactor; // makes cubes closer to the camera bigger
    vertsRaw[i] *= scaleFactor;
    vertsRaw[i + 1] *= scaleFactor;
  }

  var vertices = new Float32Array(vertsRaw);
  var n = vertsRaw.length/4; // number of vertices held in 'vertices' array
